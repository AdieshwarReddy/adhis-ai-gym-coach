...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        model_path = str(Path(__file__).resolve().parent.parent.parent / "ml_models" / "pose_landmarker_full.task")
    ...
    def _draw_overlays(self, img, metrics, ex_type):
        ...
        stage = (metrics.get("stage") or "ready").upper()
        
        # Primary angle mapping
        if "knee_angle" in metrics:
            angle_str = f"KNEE: {metrics['knee_angle']}°"
        elif "elbow_angle" in metrics:
            angle_str = f"ELBOW: {metrics['elbow_angle']}°"
        elif "front_knee_angle" in metrics:
            angle_str = f"KNEE: {metrics['front_knee_angle']}°"
        else:
            angle_str = ""

        # Draw semi-transparent top banner background
        cv2.rectangle(img, (0, 0), (img.shape[1], 55), (20, 20, 20), -1)
        hud_text = f"REPS: {reps}  |  STAGE: {stage}  |  {angle_str}"
        cv2.putText(
            img,
            hud_text,
            (20, 38),
